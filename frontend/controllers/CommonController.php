...
class CommonController extends Controller{
    ...
    public function beforeAction($action)
    {
        ...
        $this->_organizationIds = $user['organization_ids'];
      ...
      //  }
        ...
        if (Yii::$app->user->can($action)) {
            ...
            $this->error('Sorry. You do not have permission to do this.');
        }
    }
    ...
    private function dispatchJump($message,$status=1,$jumpUrl='',$ajax=false) {
        ...
        if($status) { //发送成功信息
            $message = $message ? $message : 'Submit success' ;// 提示信息
            // 默认操作成功自动返回操作前页面
            ...
            $message = $message ? $message : 'Error Occours' ;// 提示信息
            // 默认发生错误的话自动返回上页
