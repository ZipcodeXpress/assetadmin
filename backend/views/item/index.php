    ...
    <div class="ibox-content">
        <div class="row">
            <div class="col-sm-6 m-b-xs">
                <?=Html::a("Create <i class='icon-plus'></i>" , ['create' ], ['class' => 'btn btn-sm btn-primary'])?>
                ...
                <?php else: ?>
                    ...
                    <?php foreach ( $data['data'] as $list):?>
                        <tr data-key =" <?=$list[ 'name'] ?>" >
                            ...
                            <td ><?= Html::encode(($list[ 'type']==1)? 'Role': 'Permission') ?></ td>
                            <td ><?= Html::encode($list[ 'description']) ?></ td>
                            <td> <?=Html:: encode($list['created_at']?date('Y-m-d',$list['created_at']):'未设置') ?></ td>
                            <td >
                                <a class="btn btn-primary btn-xs" href= "<?= Url::to([ 'update', 'id'=>$list[ 'name']]); ?>" ><i class="fa fa-edit"></i>Edit </a >
                                <?php if($type == 1):?>
                                <a class="btn btn-success btn-xs" href= "<?= Url::toRoute([ 'item/set', 'role'=>$list[ 'name']]); ?>" ><i class="fa fa-unlock-alt"></i>Permission </a >
                                <?php endif;?>
                                <?php if($list['name'] != 'SuperAdmin'):?>
                                <a class ="del btn btn-default btn-xs" href= "javaScript:;"><i class="fa fa-close"></i>Delete</a >
                                <?php endif;?>
                            </td >
                        </tr >
                    <?php endforeach; ?>
